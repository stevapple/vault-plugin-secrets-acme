...
- `key_type` `(string: "RSA2048")` - The type of private key generated for each certificate: one of `EC256`, `EC384`, `RSA2048`, `RSA4096` or `RSA8192`, named as for an account's `key_type`. The default is what every certificate was issued with before the setting existed. Changing it on a role orders new certificates; cached ones with the previous key type are not served for it again.
